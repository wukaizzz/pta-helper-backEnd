...
app.get('/config', (req, res) => {
    const doubaoApiKey = process.env.ARK_API_KEY;
    ...
    if (doubaoApiKey) {
        ...
        res.json({
            ...
            apiUrl: process.env.ARK_API_URL || 'https://ark.cn-beijing.volces.com/api/v3/chat/completions',
            maxRequests: process.env.MAX_REQUESTS_PER_MINUTE || 10
        });
    } else {
        // 回退到 OpenAI 兼容 API 配置
        res.json({
            provider: 'openai',
...
async function callAIAPI(systemPrompt, userMessage) {
    try {
        // 优先使用豆包 API
        const doubaoApiKey = process.env.ARK_API_KEY;
        const doubaoModel = process.env.ARK_MODEL || 'doubao-seed-2-0-lite-260215';
        const doubaoApiURL = process.env.ARK_API_URL || 'https://ark.cn-beijing.volces.com/api/v3/chat/completions';

        if (doubaoApiKey) {
            console.log('使用豆包 API 进行请求');
            const response = await axios.post(doubaoApiURL, {
                model: doubaoModel,
                messages: [
        ...
        if (!apiKey) {
            throw new Error('未配置 AI_API_KEY 或 ARK_API_KEY');
        }

        console.log('使用 OpenAI 兼容 API 进行请求');
